...
Tilemap::Tilesheet::Tilesheet(const std::string &tmj_path, const std::string &png_path) {
...
const sf::Texture &Tilemap::Tilesheet::getTexture() const {
    return this->texture;
}

sf::Vector2f Tilemap::Tilesheet::getTextureCoord(const int &idx) const {
    return {
    ...
    for (auto &chunk_row: this->chunks) {
        for (auto &chunk: chunk_row.second) {
            for (auto &layer: chunk.second) {
                window.draw(*layer.second, &this->tileSheet.getTexture());
            }
        }
    }
}
